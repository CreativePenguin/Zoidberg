 ...
 * PIDOutput
 */
public class LocalPIDController implements LiveWindowSendable, Controller {
	...
	private double m_izone = Integer.MAX_VALUE;
	...
	@SuppressWarnings("unused")
	private boolean m_controlAcceleration;
	...
	private class PIDTask extends TimerTask {
		...
		private LocalPIDController m_controller;

		public PIDTask(LocalPIDController controller) {
			if (controller == null) {
	 ...
	 */
	public LocalPIDController(double Kp, double Ki, double Kd, double Kf,
	 ...
	 * integral and differential terms. The default is 50ms.
	 */
	public LocalPIDController(double Kp, double Ki, double Kd,
			PIDSource source, PIDOutput output,
			double period) {
	...
	public void setControlAcceleration(boolean controlAcceleration) {
		this.m_controlAcceleration = controlAcceleration;
	}
	...
	private void calculate() {
		...
		if (enabled) {
			...
			synchronized (this) {
				...
				if (m_I != 0) {
					if (m_izone != Integer.MAX_VALUE)  {
						if (Math.abs(m_error) < m_izone) m_totalError += m_error;
						else m_totalError = 0;
					}
					else {
	...
	public synchronized void setIZone (double pIzone) {
		m_izone = pIzone;
	}

	public synchronized void setDeadband (double pDeadband) {
	...
	public synchronized double getITerm() {
		return m_ITerm;
	}
